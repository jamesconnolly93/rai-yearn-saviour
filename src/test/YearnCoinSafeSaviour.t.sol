...
import {YErc20, YVault} from "../integrations/yearn/YErc20.sol";
import {ComptrollerG2} from "../integrations/yearn/ComptrollerG2.sol";
import {Unitroller} from "../integrations/yearn/Unitroller.sol";
import {WhitePaperInterestRateModel} from "../integrations/yearn/WhitePaperInterestRateModel.sol";
import {PriceOracle} from "../integrations/yearn/PriceOracle.sol";

import {SaviourCRatioSetter} from "../SaviourCRatioSetter.sol";
import {SAFESaviourRegistry} from "../SAFESaviourRegistry.sol";

import {YearnCoinSafeSaviour} from "../saviours/YearnCoinSafeSaviour.sol";

abstract contract Hevm {
    function warp(uint256) virtual public;
}
contract YearnPriceOracle is PriceOracle {
    uint256 price;
    ...
    function getUnderlyingPrice(YVault yVault) override external view returns (uint) {
        return price;
    }
}
contract Feed {
...
contract TestSAFEEngine is SAFEEngine {
    ...
    function deposit(address usr, uint wad) public {
...
contract FakeUser {
    ...
    function doWithdraw(
        ...
        saviour.withdraw(collateralType, safeID, yvTokenAmount, dst);
    }
...
contract YearnCoinSafeSaviourTest is DSTest {
    Hevm hevm;
    ...
    YearnPriceOracle yearnSysCoinOracle;
    ...
    // yearn Params
    uint256 systemCoinsTodeposit = 100000 * 10**18;
    ...
    uint8 yVaultDecimals = 8;

    string yVaultSymbol = "yvRAI";
    string yVaultName = "yvRAI";
    ...
    uint256 goldTodeposit = 5000 ether;
    ...
    function setUp() public {
        ...
        systemCoin.mint(address(this), systemCoinsTodeposit);
        systemCoinOracle = new Feed(systemCoinPrice, true);

        // yearn setup
        yearnSysCoinOracle = new YearnPriceOracle();
        yearnSysCoinOracle.setPrice(systemCoinPrice);
        ...
        comptroller._setPriceOracle(yearnSysCoinOracle);
        ...
        comptroller._supportMarket(yvRAI);
        ...
        saviour = new YearnCoinSafeSaviour(
            address(coinJoin),
    ...
    function default_save(uint256 safe, address safeHandler, uint desiredCRatio) internal {
        ...
        assertTrue(oldyVaultSupply - yvRAI.totalSupply() > 0);
        assertTrue(yvRAI.totalSupply() > 0);
    ...
    function default_create_liquidatable_position_deposit_cover(uint256 desiredCRatio, uint256 liquidatableCollateralPrice) internal returns (address) {
        ...
        uint256 totalSupply = yvRAI.totalSupply();
        assertTrue(totalSupply > 0);
        assertEq(yvRAI.balanceOf(address(saviour)), totalSupply);
        assertEq(systemCoin.balanceOf(address(yvRAI)), defaultTokenAmount);
        assertEq(saviour.yvTokenCover("gold", safeHandler), totalSupply);
    ...
    function test_setup() public {
        ...
        assertEq(address(saviour.yVault()), address(yvRAI));
    }
    ...
    function test_deposit_no_prior_yearn_liquidity() public {
    ...
    function test_deposit_twice() public {
        ...
        assertTrue(totalSupply > 0);
        assertEq(yvRAI.balanceOf(address(saviour)), totalSupply);
        assertEq(systemCoin.balanceOf(address(yvRAI)), defaultTokenAmount * 2);
        assertEq(saviour.yvTokenCover("gold", safeHandler), totalSupply);
    }
    function test_deposit_after_everything_withdrawn() public {
        ...
        totalSupply = yvRAI.totalSupply();
        assertTrue(totalSupply == 0);
        assertEq(yvRAI.balanceOf(address(saviour)), 0);
        assertEq(systemCoin.balanceOf(address(yvRAI)), 0);
        assertEq(saviour.yvTokenCover("gold", safeHandler), 0);
    ...
    function test_withdraw_twice() public {
        ...
        assertTrue(totalSupply > 0);
        assertEq(yvRAI.balanceOf(address(saviour)), totalSupply / 2);
        assertEq(systemCoin.balanceOf(address(yvRAI)), defaultTokenAmount / 2);
        assertEq(saviour.yvTokenCover("gold", safeHandler), totalSupply / 2);
    ...
    function test_canSave_insufficient_yVault_cover() public {
    ...
    function test_saveSAFE_insufficient_yVault_coverage() public {
        hevm.warp(now + 1);
    ...
    function test_saveSAFE_withdraw() public {
        ...
        // Withdraw
        alice.doWithdraw(saviour, "gold", safe, saviour.yvTokenCover("gold", safeHandler), address(this));
